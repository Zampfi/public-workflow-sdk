...
class TestTraceActivityInterceptor:
    ...
    @pytest.mark.asyncio
    async def test_execute_activity_with_trace(self, mock_next_activity, context_bind_fn, trace_context, mock_payload_converter):
...
class TestTraceWorkflowInboundInterceptor:
    ...
    @pytest.mark.asyncio
    async def test_execute_workflow_with_trace_header(self, mock_next_workflow_inbound, context_bind_fn, mock_logger, mock_payload_converter):
        with patch.object(workflow, "payload_converter", return_value=mock_payload_converter), \
             ...
             patch.object(workflow.unsafe, "sandbox_unrestricted", return_value=MagicMock(__enter__=MagicMock(), __exit__=MagicMock())):
    ...
    @pytest.mark.asyncio
    async def test_execute_workflow_without_trace_header(self, mock_next_workflow_inbound, context_bind_fn, mock_logger):
        with patch.object(workflow, "info", return_value=MagicMock(workflow_id="wf-123")), \
             patch.object(workflow.unsafe, "sandbox_unrestricted", return_value=MagicMock(__enter__=MagicMock(), __exit__=MagicMock())):
