...
async def main():
    service = await TemporalService.connect(
        TemporalClientConfig(host="localhost:7233", namespace="default", is_cloud=False)
    )
    activities = [
        Activity(name="generate_joke", func=generate_joke),
        Activity(name="human_approval", func=human_approval),
        Activity(name="final_activity", func=final_activity),
    ]
