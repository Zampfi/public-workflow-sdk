...
@pytest.fixture
def sample_node_id():
    """Sample node_id for integration testing."""
    return "integration_test_action#instance_456"


@pytest.fixture
def node_id_dict(sample_node_id):
    """Sample node_id dictionary."""
...
@pytest.fixture
def main_interceptor():
    """Create main NodeIdInterceptor instance."""
    return NodeIdInterceptor()


@pytest.fixture
def outbound_interceptor(mock_next_outbound):
    """Create NodeIdWorkflowOutboundInterceptor instance."""
    return NodeIdWorkflowOutboundInterceptor(mock_next_outbound)


# Integration tests
def test_end_to_end_activity_flow(main_interceptor, mock_next_outbound, sample_node_id, node_id_dict):
    ...
    mock_converter.to_payload.return_value = f"payload:{sample_node_id}"
    ...
    with patch.object(workflow, "payload_converter", return_value=mock_converter):
        ...
        activity_input.args = ("param1", node_id_dict, "param2")
...
@pytest.mark.asyncio
async def test_end_to_end_child_workflow_flow(main_interceptor, sample_node_id, node_id_dict):
    ...
    with patch.object(workflow, "payload_converter", return_value=mock_converter):
        ...
        child_input.headers = {}
        child_input.args = ("workflow_param1", node_id_dict, "workflow_param2")
...
def test_interceptor_chain_creation(main_interceptor):
    """Test that the interceptor chain is created correctly."""
    # Create main interceptor with logger
    mock_logger = MagicMock()
    main_interceptor_custom = NodeIdInterceptor(logger_module=mock_logger)

    # Create the interceptor chain
    interceptor_class = main_interceptor_custom.workflow_interceptor_class(MagicMock())
    mock_next_inbound = MagicMock(spec=WorkflowInboundInterceptor)
    inbound_interceptor = interceptor_class(mock_next_inbound)
...
def test_multiple_activities_in_sequence(outbound_interceptor, mock_payload_converter):
    ...
    with patch.object(workflow, "payload_converter", return_value=mock_payload_converter):
        ...
        activity2_input.args = ("param", {"__temporal_node_id": "action2#instance2"})
...
@pytest.mark.asyncio
async def test_mixed_activities_and_child_workflows(mock_next_outbound, mock_payload_converter):
...
def test_no_node_id_handling(outbound_interceptor):
    ...
    outbound_interceptor.next.start_activity.assert_called_once()


def test_malformed_node_id_dict(outbound_interceptor):
...
def test_custom_logger_integration():
...
def test_node_id_header_constant():
