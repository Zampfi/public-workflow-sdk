...
class TestNodeIdInterceptorIntegration:
    ...
    def test_multiple_activities_in_sequence(self):
        ...
        outbound_interceptor = NodeIdWorkflowOutboundInterceptor(
            mock_next_outbound,
            main_interceptor.node_id_header_key
        )
...
if __name__ == "__main__":
    ...
    class SimpleTestRunner:
        ...
        def run_test(self, test_method, test_name):
            try:
                test_instance = TestNodeIdInterceptorIntegration()
        ...
        async def run_async_test(self, test_method, test_name):
            try:
                test_instance = TestNodeIdInterceptorIntegration()
                test_instance.setup_method()
    ...
    print("Running NodeIdInterceptor Integration Tests...")
    print("=" * 50)
    
    # Run synchronous tests
    runner.run_test(
        TestNodeIdInterceptorIntegration.test_end_to_end_activity_flow,
        "test_end_to_end_activity_flow"
    )
    
    runner.run_test(
        TestNodeIdInterceptorIntegration.test_interceptor_chain_creation,
        "test_interceptor_chain_creation"
    )
    
    runner.run_test(
        TestNodeIdInterceptorIntegration.test_multiple_activities_in_sequence,
        "test_multiple_activities_in_sequence"
    )
    
    runner.run_test(
        TestNodeIdInterceptorIntegration.test_mixed_activities_and_child_workflows,
        "test_mixed_activities_and_child_workflows"
    )
    
    runner.run_test(
        TestNodeIdInterceptorIntegration.test_no_node_id_handling,
        "test_no_node_id_handling"
    )
    
    runner.run_test(
        TestNodeIdInterceptorIntegration.test_malformed_node_id_dict,
        "test_malformed_node_id_dict"
    )
    
    runner.run_test(
        TestNodeIdInterceptorIntegration.test_custom_logger_integration,
        "test_custom_logger_integration"
    )
    
    runner.run_test(
        TestNodeIdInterceptorIntegration.test_node_id_header_constant,
        "test_node_id_header_constant"
    )
    
    print("=" * 50)
    print(f"Tests completed: {runner.tests_passed}/{runner.tests_run} passed")
    
    if runner.tests_passed == runner.tests_run:
